...
(async()=>{
 ...
 assert.equal(w.document.querySelector('.rs-entry'),null,'designing starts without the obsolete Early Access interstitial');
 assert.equal(w.document.querySelector('.rs-beta'),null);
 w.RentSketchCustomerEntry.terms();assert.match(w.document.querySelector('.rs-modal').textContent,/Lakeside Events/);assert.doesNotMatch(w.document.querySelector('.rs-modal').textContent,/Friendly Party Rental|Early Access/);w.document.querySelector('.rs-modal [data-close]').click();
 ...
 console.log('PASS: delayed second-tenant hydration → exact 2D preview → no welcome interruption → own equipment/prices → complete review → mocked quote reuses same tenant design; no network writes');
 dom.window.close();
