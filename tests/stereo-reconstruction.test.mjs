...
test('edge-aware smoothing reduces local depth noise without washing across a depth boundary',()=>{
  ...
  depths[1*cols+1]=22.4;depths[2*cols+4]=37.8;
  const result=smoothDepthField({depths,confidence,valid,cols,rows,strength:.45,edgeFraction:.08,edgeFt:1.5,iterations:1});
  assert.ok(Math.abs(result.depths[1*cols+1]-20)<Math.abs(22.4-20),'left-side depth noise moves toward its local surface');
  assert.ok(Math.abs(result.depths[2*cols+4]-40)<Math.abs(37.8-40),'right-side depth noise moves toward its local surface');
