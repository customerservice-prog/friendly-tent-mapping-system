...
async function fixture({fail=false,page='business/index.html',reduced=true}={}){
...
(async()=>{
 ...
 assert.equal(f.metrics().imports,0,'supporting page keeps fast static first frame');
 ...
 assert.equal(f.metrics().cameras[0],'outside','opens entire furnished tent');
 ...
 const eventPass=await fixture({page:'event-pass/index.html'});
 eventPass.intersect();await settle();
 assert.equal(eventPass.metrics().created,0,'Event Pass keeps renderer off the critical path');
 eventPass.dom.window.close();

 console.log('PASS 3D marketing tour: shared complete wedding layout, explicit 3D activation on supporting pages, camera controls, 2D switching, WebGL fallback, and history cleanup. Renderer stub; not GPU visual QA.');
