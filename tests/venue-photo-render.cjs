...
(async()=>{
 ...
 assert.ok(scene.getObjectByName('Photo world derived ground'),'360 world builds a photo-derived ground material');
 assert.ok(scene.getObjectByName('Photo world panoramic shell'),'360 world builds a continuous panoramic surround');
 assert.ok(scene.getObjectByName('Photo world parallax layer'),'360 world creates image-derived depth layers for parallax');
 assert.ok(local360.userData.layers.parallax>=10,'360 world keeps multiple stable parallax layers');
 ...
 dom.window.close();
 console.log('PASS venue photo renderer: locked Matched View plus local no-API spatial 360 world with photo-derived ground, panorama, parallax and eye-level camera.');
