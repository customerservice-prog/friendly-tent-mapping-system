...
(async()=>{
 ...
 assert.ok(local360&&!local360.visible,'locally synthesized side/rear environment stays hidden in Matched View');
 assert.equal(local360.userData.noExternalApi,true,'Smart 360 synthesis is explicitly local and API-free');
 assert.ok(scene.getObjectByName('Local Smart 360 left'),'local synthesis creates a left continuation');
 assert.ok(scene.getObjectByName('Local Smart 360 right'),'local synthesis creates a right continuation');
 assert.ok(scene.getObjectByName('Local Smart 360 rear'),'local synthesis creates a rear continuation');
 ...
 assert.equal(local360.visible,true,'Smart 360 reveals the browser-synthesized side and rear environment');
 ...
 assert.equal(local360.visible,false,'returning to Matched View hides local synthesized unseen directions');
 ...
 dom.window.close();
 console.log('PASS venue photo renderer: locked Matched View, no-API local Smart 360 side/rear synthesis, fixed world continuation, projected photo stage and calibrated camera.');
