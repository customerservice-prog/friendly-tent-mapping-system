...
const db={query:async(sql,args)=>{if(sql.startsWith('SELECT id FROM tenants'))return{rows:[{id:'tenant-fixture'}]};writes.push({sql,args});return{rows:[{id:'design-fixture'}]};}};
const context={Buffer,console,module:{exports:{}},require:name=>name==='express'?{Router:()=>router}:name==='../db'?db:name.includes('requireAuth')?{requireTenantAccess(){}}:(()=>{throw Error('Unexpected dependency '+name);})()};
