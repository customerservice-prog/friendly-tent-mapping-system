...
(async()=>{
 ...
 let r=await req('/api/tenants/friendly/designs/'+design.id+'/share',{body:{anonymousSessionId:'owner-session',expiresInDays:180}});
 ...
 assert.equal((await req('/api/tenants/other/shared-design/restore',{body:{token}})).status,404,'share token is tenant-bound');
 assert.equal((await req('/api/tenants/friendly/shared-design/restore',{body:{token:token.slice(0,-1)+'x'}})).status,404,'tampered opaque share token is rejected');
 console.log('PASS design sharing: owner-only creation, 180-day revocable opaque fragment URL, exact sidewall scene restore, tenant binding and tamper rejection.');
