...
(async()=>{
 ...
 // Sidewalls are selectable per side, persist in scene state, render in 2D, and appear in review line items.
 click('[data-drawer="tent"]');assert.ok(d.querySelector('[data-role="sidewall-all"][data-type="solid"]'));assert.ok(d.querySelector('[data-role="sidewall-all"][data-type="window"]'));
 click('[data-role="sidewall-all"][data-type="solid"]');assert.equal(b.getScene().sidewalls.length,8,'20x20 perimeter uses eight 10-ft sidewall segments');assert.ok(b.getScene().sidewalls.every(x=>x.type==='solid'));assert.equal(d.querySelectorAll('.plan2d-sidewall.solid').length,8);
 const frontWall=d.querySelector('[data-role="sidewall-side"][data-side="front"]');frontWall.value='window';frontWall.dispatchEvent(new w.Event('change',{bubbles:true}));
 assert.equal(b.getScene().sidewalls.filter(x=>x.side==='front'&&x.type==='window').length,2);assert.equal(d.querySelectorAll('.plan2d-sidewall.window').length,2);
 let wallLines=b.computeLineItems().filter(l=>l.category==='sidewall');assert.equal(wallLines.find(l=>/Solid/.test(l.label)).qty,6);assert.equal(wallLines.find(l=>/Window/.test(l.label)).qty,2);
 click('[data-role="sidewall-all"][data-type="none"]');assert.equal(b.getScene().sidewalls.length,0,'sidewalls can be removed without changing the tent');assert.equal(b.computeLineItems().filter(l=>l.category==='sidewall').length,0);
 ...
 console.log('PASS customer experience: surfaces/ballast, solid + window sidewalls with saved state and 2D summary, standalone throne controls, lighting, help, quick share, delivery/tax review and isolated quote payload. No live writes.');w.close();
