...
(async()=>{
 ...
 view.rebuild({...data,sidewalls:{front:'solid',right:'window',back:'none',left:'solid'}});view.setScene({night:true,weather:'rain',guests:true,motion:true});
 const scene=renderer.scene;let solidWalls=0,windowWalls=0;scene.traverse(o=>{if(o.userData.sidewallStyle==='solid')solidWalls++;if(o.userData.sidewallStyle==='window')windowWalls++;});assert.ok(solidWalls>=4,'solid sidewalls render as segmented tent panels');assert.ok(windowWalls>=2,'window sidewalls render as transparent segmented panels');assert.equal(scene.fog.density,.004,'rain softens the background');
