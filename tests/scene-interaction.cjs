...
(async()=>{
 ...
 view.setScene({night:true,weather:'clear',guests:false,motion:false});assert.ok(scene.getObjectByName('Moon').visible);assert.ok(!scene.getObjectByName('Preview guests').visible);
