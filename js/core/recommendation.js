// RentSketch multi-tenant tent recommendation engine.
// Takes event intake answers and suggests RECOMMENDED / TIGHTER / MORE SPACIOUS tents.
// This produces planning guidance only; the active rental provider confirms final installation.
...
function providerName() {
  return (typeof window !== 'undefined' && window.ACTIVE_TENANT && window.ACTIVE_TENANT.name) || 'your rental company';
}
...
function surfaceWarning(tent, surfaceType) {
  ...
  if (tent.type === 'pole' && hardSurfaces.indexOf(surfaceType) !== -1) {
    return { level: 'warning', message: 'This tent type typically requires staking. On this surface, ballast or a frame tent may be required — ' + providerName() + ' will verify final installation requirements.' };
  }
  if (surfaceType === 'notSure' || !surfaceType) {
    return { level: 'info', message: 'Installation method will be confirmed by ' + providerName() + ' based on your exact surface.' };
  }
  return { level: 'info', message: tent.surfaceNotes };
}
function normalizeTenantTent(t, tenant) {
  var dining = (t.maxGuests && t.maxGuests.dining) || (t.capacity && t.capacity.diningRounds) || 0;
  var cocktail = (t.maxGuests && t.maxGuests.cocktail) || (t.capacity && t.capacity.cocktail) || dining;
  return Object.assign({}, t, {
    active: t.active !== false,
    capacity: t.capacity || { diningRounds: dining, ceremonyRows: dining, cocktail: cocktail },
    surfaceNotes: t.surfaceNotes || ('Installation surface requirements will be confirmed by ' + ((tenant && tenant.name) || 'your rental company') + '.'),
  });
}
function eligibleTentsForActiveTenant() {
  var tenant = (typeof window !== 'undefined') ? window.ACTIVE_TENANT : null;
  if (tenant && tenant.slug && tenant.slug !== 'generic' && Array.isArray(tenant.tents)) {
    return tenant.tents.filter(function (t) { return t && t.active !== false; }).map(function (t) { return normalizeTenantTent(t, tenant); });
  }
  if (tenant && tenant.slug === 'generic') {
    return TENTS.filter(function (t) { return t.active; }).map(function (t) { return Object.assign({}, t, { pricePerDay: null }); });
  }
  return TENTS.filter(function (t) { return t.active; });
}
function safePrice(t) {
  return (t && t.pricePerDay !== null && t.pricePerDay !== undefined && Number.isFinite(Number(t.pricePerDay))) ? Number(t.pricePerDay) : Infinity;
}
...
export function recommendTents(input) {
  ...
  const eligible = eligibleTentsForActiveTenant().slice().filter(function (t) { return t.capacity && Number.isFinite(Number(t.capacity[capacityKey])); }).sort(function (a, b) { return a.capacity[capacityKey] - b.capacity[capacityKey]; });

  const ROUND_TABLE_SEATS = 8;
  const ROUND_TABLE_CELL_FT = 10.7;
  const GRID_START_MARGIN_FT = 3;
  const GRID_TABLE_FOOTPRINT_FT = Math.max(0, ROUND_TABLE_CELL_FT - 3.5);
  function diningRoundsGridCapacity(tent, activeFeatures, danceFloorSizeId, customDanceFloorFt) {
    var usableLengthFt = tent.lengthFt;
    if (activeFeatures && activeFeatures.indexOf('danceFloor') !== -1) {
  ...
  if (viableTents.length > 0) {
    const cheapestViable = viableTents.slice().sort(function (a, b) {
      const aPrice = safePrice(a), bPrice = safePrice(b);
  ...
  if (recommendedIndex === -1) {
    ...
    result.warnings.push({ level: 'error', message: 'No single tent in current inventory comfortably fits this event. ' + providerName() + ' can help plan a multi-tent or custom layout.' });
  ...
  if (recommendedIndex > 0) {
    ...
    if (smaller.capacity[capacityKey] >= guestCount * 0.9) {
      ...
      result.tighter = { tent: smaller, note: surfaceWarning(smaller, surfaceType), caution: smallerIsViable ? undefined : 'May fit with reduced comfort and fewer feature areas than requested.' };
    }
  }
  if (recommendedIndex < eligible.length - 1) {
    const bigger = eligible[recommendedIndex + 1];
    const moreExpensive = safePrice(bigger) > safePrice(recommendedTent);
    result.moreSpacious = { tent: bigger, note: surfaceWarning(bigger, surfaceType), benefit: moreExpensive ? 'More comfortable circulation and room to grow the layout.' : 'More comfortable circulation and room to grow the layout, with no higher listed daily price.' };
  }
  return result;
}
