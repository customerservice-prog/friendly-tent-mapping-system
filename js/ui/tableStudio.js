...
function ensure(){
 ...
 dialog.addEventListener('change',e=>{const el=e.target,id=el.dataset.product;if(el.dataset.ts==='per-seat')editTop(id,(list,entry)=>{if(entry){entry.qty=tabletopQuantity(entry,item());entry.perSeat=el.checked;}});else if(el.dataset.ts==='top-color')editTop(id,(list,entry)=>{if(entry)entry.color=el.value;});else if(el.dataset.role==='insp-chair')change({chairId:el.value});else if(el.dataset.role==='insp-linen')setLinen(el.value);});
