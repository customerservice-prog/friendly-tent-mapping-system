...
var KEY='rentsketch-autosave:'+slug,SESSION='rentsketch-anon-session',timer=null,saving=false,lastJson='',lastId=null,retryAfter=0,pending=false,ownedSession=null;
...
// Read the current login for each request, including retries after a missing draft.
function saveHeaders(){var headers={'Content-Type':'application/json'};try{var token=localStorage.getItem('rentsketch_dashboard_token');if(token)headers.Authorization='Bearer '+token;}catch(e){}return headers;}
async function sendSave(sc,m,sid,id){var base=api+(slug==='generic'?'/api/consumer/designs':'/api/tenants/'+encodeURIComponent(slug)+'/designs'),isUpdate=!!(id&&sid),r=await fetch(isUpdate?base+'/'+encodeURIComponent(id):base,{method:isUpdate?'PATCH':'POST',headers:saveHeaders(),body:payload(sc,m,sid)});if(isUpdate&&r.status===404){lastId=null;r=await fetch(base,{method:'POST',headers:saveHeaders(),body:payload(sc,m,sid)});}return r;}
