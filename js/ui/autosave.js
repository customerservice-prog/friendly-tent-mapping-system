...
async function sendSave(sc,m,sid,id){var base=api+(slug==='generic'?'/api/consumer/designs':'/api/tenants/'+encodeURIComponent(slug)+'/designs'),isUpdate=!!(id&&sid),r=await fetch(isUpdate?base+'/'+encodeURIComponent(id):base,{method:isUpdate?'PATCH':'POST',headers:{'Content-Type':'application/json'},body:payload(sc,m,sid)});if(isUpdate&&r.status===404){lastId=null;r=await fetch(base,{method:'POST',headers:{'Content-Type':'application/json'},body:payload(sc,m,sid)});}return r;}
