...
var bootStarted=false;setTimeout(function(){try{var ca=document.querySelector('.canvas-area'),cv=document.querySelector('.canvas-viewport'),ds=document.querySelector('.designer-shell'),sd=document.getElementById('step-designer'),r1=ca?ca.getBoundingClientRect():0,r2=cv?cv.getBoundingClientRect():0,r3=ds?ds.getBoundingClientRect():0,r4=sd?sd.getBoundingClientRect():0;var info='area:'+(ca?Math.round(r1.width)+'x'+Math.round(r1.height):'MISSING')+' vp:'+(cv?Math.round(r2.width)+'x'+Math.round(r2.height):'MISSING')+' shell:'+(ds?Math.round(r3.width)+'x'+Math.round(r3.height):'MISSING')+' stepDes:'+(sd?Math.round(r4.width)+'x'+Math.round(r4.height):'MISSING')+' bodyH:'+Math.round(document.body.getBoundingClientRect().height)+' winH:'+window.innerHeight+' canvases:'+document.querySelectorAll('canvas').length;var box=document.createElement('div');box.style.cssText='position:fixed;top:0;left:0;right:0;background:red;color:#fff;font:11px monospace;padding:4px;z-index:999999;white-space:pre-wrap';box.textContent=info;document.body.appendChild(box);}catch(e){var box=document.createElement('div');box.style.cssText='position:fixed;top:0;left:0;right:0;background:red;color:#fff;font:11px monospace;padding:4px;z-index:999999';box.textContent='diag error: '+e.message;document.body.appendChild(box);}},4000);setTimeout(function(){try{var dm=document.querySelector('.designer-main'),ca2=document.querySelector('.canvas-area'),sheets=Array.prototype.map.call(document.styleSheets,function(s){var n;try{n=(s.href?s.href.split('/').pop():'inline')+':'+(s.cssRules?s.cssRules.length+'rules':'BLOCKED');}catch(e){n=(s.href||'inline')+':ERR';}return n;}).join('|');var info2='winW:'+window.innerWidth+' mainDisp:'+(dm?getComputedStyle(dm).display:'NOMAIN')+' areaW:'+(ca2?getComputedStyle(ca2).width:'NOAREA')+' areaFlexGrow:'+(ca2?getComputedStyle(ca2).flexGrow:'?')+' rsInk:'+getComputedStyle(document.documentElement).getPropertyValue('--rs-ink')+' sheets:'+sheets;var box2=document.createElement('div');box2.style.cssText='position:fixed;top:20px;left:0;right:0;background:blue;color:#fff;font:10px monospace;padding:4px;z-index:999999;white-space:pre-wrap';box2.textContent=info2;document.body.appendChild(box2);}catch(e){var box2=document.createElement('div');box2.style.cssText='position:fixed;top:20px;left:0;right:0;background:blue;color:#fff;font:10px monospace;padding:4px;z-index:999999';box2.textContent='diag2 error: '+e.message;document.body.appendChild(box2);}},4500);
