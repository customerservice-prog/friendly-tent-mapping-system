...
function waitForVisible3d(b,tent,attempt){attempt=attempt||0;var btn=document.getElementById('viewMode3d');if(!btn){if(attempt<80)return setTimeout(function(){waitForVisible3d(b,tent,attempt+1);},50);return fail('3D view control did not initialize');}if(attempt===0)btn.click();requestAnimationFrame(function(){requestAnimationFrame(function(){var canvas=document.querySelector('canvas');var host=canvas&&canvas.parentElement;var r=(host||canvas)&&((host||canvas).getBoundingClientRect());if(!r||r.width<2||r.height<2){if(attempt<120)return setTimeout(function(){waitForVisible3d(b,tent,attempt+1);},50);return fail('3D viewport did not become visible');}try{window.dispatchEvent(new Event('resize'));if(b.fitTentPreview)b.fitTentPreview();}catch(e){console.warn('Tent preview fit retry failed',e);}setTimeout(function(){try{window.dispatchEvent(new Event('resize'));if(b.fitTentPreview)b.fitTentPreview();}catch(e){}ready(tent);},150);});});}
var tries=0;
