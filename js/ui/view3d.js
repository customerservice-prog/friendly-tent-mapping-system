...
function makeTent(t,anchor,sidewalls={}){const p=structuralProfile(t.type,t.widthFt,t.lengthFt),g=new THREE.Group(),hw=t.widthFt/2,hl=t.lengthFt/2,vinyl=vinylTexture();g.userData.kind='tent';const roof=makeRoof(t,p,vinyl);g.add(roof);const steel=new THREE.MeshStandardMaterial({color:0xb8bec3,roughness:.24,metalness:.82}),black=new THREE.MeshStandardMaterial({color:0x303236,roughness:.62}),strap=new THREE.MeshStandardMaterial({color:0xe5dfd0,roughness:.92}),val=new THREE.MeshPhysicalMaterial({color:0xfffdf8,map:vinyl,roughness:.65,side:THREE.DoubleSide});const stations=computePerimeterStations(t.widthFt,t.lengthFt).map(s=>[s.x-hw,s.y-hl]);stations.forEach(([x,z])=>{const pole=cyl(p.sidePoleDiameterFt/2,p.eaveHeightFt,steel,16);pole.position.set(x,p.eaveHeightFt/2,z);pole.castShadow=true;pole.userData.buildStage='frame';g.add(pole);const foot=cyl(.15,.035,black,16);foot.position.set(x,.018,z);g.add(foot)});if(t.type==='pole')peakPoints(t).forEach(q=>{const cp=cyl(p.centerPoleDiameterFt/2,p.peakHeightFt+.12,steel,20);cp.position.set(q.x,(p.peakHeightFt+.12)/2,q.y);cp.castShadow=true;cp.userData.buildStage='frame';g.add(cp);const cap=cyl(.13,.12,steel,20);cap.position.set(q.x,p.peakHeightFt+.12,q.y);g.add(cap)});const drop=p.valanceDropFt;[[0,-hl,t.widthFt,.045],[0,hl,t.widthFt,.045],[-hw,0,.045,t.lengthFt],[hw,0,.045,t.lengthFt]].forEach(v=>{const q=box(v[2],drop,v[3],val);q.position.set(v[0],p.eaveHeightFt-drop/2,v[1]);q.castShadow=true;q.userData.buildStage='valance';g.add(q)});const wallSolid=new THREE.MeshPhysicalMaterial({color:0xfffdf8,roughness:.72,side:THREE.DoubleSide,transparent:true,opacity:.96}),wallClear=new THREE.MeshPhysicalMaterial({color:0xdfefff,roughness:.12,metalness:0,transparent:true,opacity:.32,side:THREE.DoubleSide,depthWrite:false}),wallTrim=new THREE.MeshStandardMaterial({color:0xf4f1ea,roughness:.78});
const wallStyles=Object.assign({front:'none',right:'none',back:'none',left:'none'},sidewalls||{});
function addWallSide(side,style){
  if(style!=='solid'&&style!=='window')return;
  const front=side==='front'||side==='back',len=front?t.widthFt:t.lengthFt,count=Math.max(1,Math.ceil(len/10)),seg=len/count;
  for(let i=0;i<count;i++){
    const center=-len/2+seg*(i+.5),thick=.06;
    if(style==='solid'){
      const q=box(front?seg:thick,p.eaveHeightFt-.12,front?thick:seg,wallSolid);
      q.position.set(front?center:(side==='left'?-hw:hw),(p.eaveHeightFt-.12)/2,front?(side==='front'?-hl:hl):center);q.userData.kind='sidewall';q.userData.sidewallStyle='solid';q.castShadow=true;q.receiveShadow=true;g.add(q);
    }else{
      const lower=box(front?seg:thick,1.9,front?thick:seg,wallTrim);lower.position.set(front?center:(side==='left'?-hw:hw),.95,front?(side==='front'?-hl:hl):center);g.add(lower);
      const clearH=Math.max(2.8,p.eaveHeightFt-3.0),clear=box(front?seg-.08:thick,clearH,front?thick:seg-.08,wallClear);clear.position.set(front?center:(side==='left'?-hw:hw),1.9+clearH/2,front?(side==='front'?-hl:hl):center);clear.userData.kind='sidewall';clear.userData.sidewallStyle='window';g.add(clear);
      const topH=Math.max(.4,p.eaveHeightFt-1.9-clearH),top=box(front?seg:thick,topH,front?thick:seg,wallTrim);top.position.set(front?center:(side==='left'?-hw:hw),1.9+clearH+topH/2,front?(side==='front'?-hl:hl):center);g.add(top);
    }
  }
}
['front','right','back','left'].forEach(side=>addWallSide(side,wallStyles[side]));
