...
export function init(container,callbacks={}) {
  ...
  function updatePhotoStageViewFade(){
    ...
    // The uploaded photo is trusted while viewing from the original/front side.
    // Fade the flat evidence plane before the user reaches a side angle so it
    // cannot turn into a giant rectangular billboard inside the 360 world.
  ...
  function frame(t){
    if(hasVenuePhoto()&&state?.photoCalibration&&cameraMode==='photo360'){
      ...
      // Keep orbiting inside the believable reconstructed bubble instead of
      // letting the camera fly far enough away to expose reconstruction seams.
  ...
  function fitCamera(){if(state?.tent){stopWalk();cameraMode='outside';syncPhotoPresentation();frame(state.tent);return true;}return false;}
  function matchPhoto(){if(!state?.tent||!hasVenuePhoto())return false;stopWalk();cameraMode='outside';syncPhotoPresentation();frame(state.photoSite||state.tent);return true;}
  function orbit360(){if(!state?.tent||!hasVenuePhoto())return false;stopWalk();cameraMode='photo360';rebuildPhotoStage();syncPhotoPresentation();frame(state.photoSite||state.tent);return true;}
  function walkWorld(){
