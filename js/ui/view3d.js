...
import * as crewMotion from './crew-motion.js';

let renderer,scene,camera,controls,root,container,ro,raf,state,night=false,ambient,hemi,sun,fill,callbacks={},raycaster,pointer,drag=null,danceMesh=null,needsRender=true;
...
function tube(a,b,r,m){const d=new THREE.Vector3().subVectors(b,a),q=cyl(r,d.length(),m,6);q.position.copy(a).add(b).multiplyScalar(.5);q.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),d.normalize());return q;}
const steel=()=>mat('steel',{color:0xbcc1c4,roughness:.3,metalness:.65}),dark=()=>mat('dark',{color:0x303437,roughness:.5}),strap=()=>mat('strap',{color:0xe4e0d4,roughness:.85});
...
function H(t){if(t.type!=='pole')return EAVE+Math.max(4,t.widthFt*.24);if(t.widthFt<=20)return EAVE+7.5;if(t.widthFt<30)return EAVE+8.5;return EAVE+10.5;}
...
let woodTex=null;function woodTexture(){if(!woodTex){const c=document.createElement('canvas');c.width=c.height=64;const ctx=c.getContext('2d');ctx.fillStyle='#a9754a';ctx.fillRect(0,0,64,64);for(let i=0;i<9;i++){const y=6+i*7+Math.random()*3;ctx.strokeStyle='rgba(60,35,15,'+(0.1+Math.random()*0.12)+')';ctx.lineWidth=1+Math.random()*2;ctx.beginPath();ctx.moveTo(0,y);ctx.bezierCurveTo(20,y+Math.random()*5-2.5,44,y+Math.random()*5-2.5,64,y);ctx.stroke();}woodTex=new THREE.CanvasTexture(c);woodTex.wrapS=woodTex.wrapT=THREE.RepeatWrapping;woodTex.repeat.set(3,3);}return woodTex;}
function centerpiece(g,cx,cz,top){const vaseM=mat('vase',{color:0xcfe8e0,roughness:.15,metalness:.1,transparent:true,opacity:.55}),stemM=mat('stem',{color:0x4a6b3a,roughness:.6}),petalM=mat('petal',{color:0xd98fa3,roughness:.6});const vase=cyl(.09,.5,vaseM,12);vase.position.set(cx,top+.25,cz);g.add(vase);for(let i=0;i<5;i++){const a=i/5*Math.PI*2,bx=cx+Math.cos(a)*.08,bz=cz+Math.sin(a)*.08;const stem=cyl(.02,.32,stemM,6);stem.position.set(bx,top+.5+.16,bz);g.add(stem);const bloom=new THREE.Mesh(new THREE.SphereGeometry(.09,8,6),petalM);bloom.position.set(bx,top+.5+.34,bz);g.add(bloom);}const center=new THREE.Mesh(new THREE.SphereGeometry(.1,8,6),petalM);center.position.set(cx,top+.5+.38,cz);g.add(center);}
function table(o){const g=new THREE.Group(),w=o.widthFt||5,d=o.depthFt||5,h=2.45,round=o.shape==='round'||Math.abs(w-d)<.2,linen=linenById(o.linenId),lc=colorFor(o.linenColor||o.color||'White'),tm=new THREE.MeshStandardMaterial({color:linen?lc:0xa9754a,map:linen?linenTexture():woodTexture(),roughness:linen?.88:.55,metalness:linen?0:.02}),legM=mat('tableLeg',{color:0x9aa0a5,roughness:.4,metalness:.35});if(round){const r=w/2;if(linen){const skirt=new THREE.Mesh(new THREE.CylinderGeometry(r,r*.94,h,24,1,true),tm);skirt.position.y=h/2;g.add(skirt);}else{const pole=cyl(r*.045,h-.12,legM);pole.position.y=(h-.12)/2;g.add(pole);const base=cyl(r*.22,.05,legM,16);base.position.y=.025;g.add(base);}const top=cyl(r,.06,tm,24);top.position.y=h;g.add(top);}else{if(linen){const q=box(w,h,d,tm);q.position.y=h/2;g.add(q);}else{const top=box(w,.06,d,tm);top.position.y=h;g.add(top);const lx=w/2-.15,lz=d/2-.15;[[lx,lz],[lx,-lz],[-lx,lz],[-lx,-lz]].forEach(([x,z])=>{const leg=box(.07,h-.06,.07,legM);leg.position.set(x,(h-.06)/2,z);g.add(leg);});}}const n=o.seatCount||0,rr=Math.max(w,d)/2+1,nc=colorFor(o.napkinColor||o.linenColor||'White');for(let i=0;i<n;i++){const a=i/n*Math.PI*2,c=chair();c.position.set(Math.cos(a)*rr,0,Math.sin(a)*rr);c.rotation.y=-a-Math.PI/2;g.add(c);if(round)placeSetting(g,Math.cos(a)*(w*.31),Math.sin(a)*(w*.31),a,nc);}centerpiece(g,0,0,h);g.userData={itemId:o.id,kind:'table',buildStage:'tables'};g.traverse(m=>m.userData.itemId=o.id);return g;}
function danceGroup(items,t){const g=new THREE.Group();if(!items.length)return g;let minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity;items.forEach(o=>{minX=Math.min(minX,o.x);minY=Math.min(minY,o.y);maxX=Math.max(maxX,o.x+o.widthFt);maxY=Math.max(maxY,o.y+o.depthFt);});const w=maxX-minX,d=maxY-minY,b=box(w,.08,d,dark());b.position.y=.04;g.add(b);const wm1=mat('wood1',{color:0xd2a15f,roughness:.28,metalness:.1}),wm2=mat('wood2',{color:0x7a4d26,roughness:.28,metalness:.1});for(let x=-w/2;x<w/2;x+=3)for(let z=-d/2;z<d/2;z+=3){const pw=Math.min(3,w-(x+w/2)),pd=Math.min(3,d-(z+d/2)),ix=Math.round((x+w/2)/3),iz=Math.round((z+d/2)/3),p=box(pw-.03,.045,pd-.03,(ix+iz)%2?wm2:wm1);p.position.set(x+pw/2,.105,z+pd/2);g.add(p);}g.position.set((minX+maxX)/2-t.widthFt/2,0,(minY+maxY)/2-t.lengthFt/2);g.userData={kind:'danceGroup',itemIds:items.map(o=>o.id),buildStage:'danceFloor'};g.traverse(m=>{m.userData.kind='danceGroup';m.userData.itemIds=g.userData.itemIds;});return g;}
function grassTexture(){const c=document.createElement('canvas');c.width=c.height=128;const ctx=c.getContext('2d');ctx.fillStyle='#87996a';ctx.fillRect(0,0,128,128);for(let i=0;i<40;i++){const x=Math.random()*128,y=Math.random()*128,r=8+Math.random()*18;ctx.fillStyle=Math.random()<.5?'rgba(255,255,255,0.05)':'rgba(30,42,20,0.06)';ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();}for(let i=0;i<1400;i++){const x=Math.random()*128,y=Math.random()*128,len=2+Math.random()*3,ang=Math.random()*Math.PI;ctx.strokeStyle=Math.random()<.5?'rgba(255,255,255,0.12)':'rgba(25,35,15,0.16)';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+Math.cos(ang)*len,y-Math.sin(ang)*len);ctx.stroke();}const tex=new THREE.CanvasTexture(c);tex.wrapS=tex.wrapT=THREE.RepeatWrapping;tex.colorSpace=THREE.SRGBColorSpace;return tex;}
...
function rebuild(d){state=d;clear();const t=d?.tent;if(!t)return;root.add(ground(t),tent(t,d.anchoringMethod));const hw=t.widthFt/2,hl=t.lengthFt/2,dance=[];(d.objects||[]).forEach(o=>{if(o.kind==='dance'){dance.push(o);return;}if(o.kind!=='table')return;const q=table(o);q.position.set(o.x+o.widthFt/2-hw,0,o.y+o.depthFt/2-hl);renderedItems.set(o.id,q);root.add(q);});if(dance.length){danceMesh=danceGroup(dance,t);root.add(danceMesh);}apply();invalidate();}
...
function loop(){raf=requestAnimationFrame(loop);controls?.update();if(needsRender){renderer?.render(scene,camera);needsRender=false;}}
function ease(t){return t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2;}

const STAGE_ORDER=['ground','frame','roof','valance','danceFloor','tables','lighting'];
const RISE_STAGES={frame:1,tables:1};
let timelapse=null,crewMeshes=[],stagingArea=null,progressOverlay=null;

function stageGroups(){const map={};STAGE_ORDER.forEach(s=>map[s]=[]);root.traverse(o=>{if(o.userData&&o.userData.buildStage&&map[o.userData.buildStage])map[o.userData.buildStage].push(o);});return map;}
