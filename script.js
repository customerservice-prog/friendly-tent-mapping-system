...
import { accessoryDrawer, accessoryInspector, accessoryIcon } from './js/ui/accessory-controls.js';
...
function pickDefaultRoundTable(){var exact=byId(TABLES,'round-5ft');if(exact)return exact;var round=TABLES.filter(function(t){return t.shape==='round'&&t.seatsDefault>0;});if(round.length)return round[0];var any=TABLES.filter(function(t){return t.seatsDefault>0;});return any.length?any[0]:(TABLES.length?TABLES[0]:null);}function useRecommendedLayout(){if(!requireEventEditing())return false;
  var tent=layoutSpace(),tableDef=pickDefaultRoundTable(),objects=store.getState().objects.filter(o=>['inflatable','accessory'].includes(o.kind));
  if(!tent)return;
...
function renderDrawerBody(kind){var body=$('drawerBody');if(kind==='site'){renderEquipmentContent(body,sitePanel(state,layoutSpace()));bindVenuePhotoInputs(body);}else if(kind==='setup')body.innerHTML=setupPanel(state,layoutSpace(),store.getState().objects.length>0);else if(kind==='inflatables')body.innerHTML=inflatableCards(INFLATABLES,store.getState().objects);else if(kind==='rentals')body.innerHTML=buildAllRentalsDrawerHtml();else if(kind==='tables')body.innerHTML=buildTablesDrawerHtml();else if(kind==='tent')body.innerHTML=buildTentDrawerHtml();else if(kind==='chairs')renderEquipmentContent(body,buildChairsDrawerHtml());else if(kind==='dance')body.innerHTML=buildDanceDrawerHtml();else if(kind==='lighting')body.innerHTML=buildLightingDrawerHtml();else body.innerHTML='<p>Choose '+kind+' options for your event.</p>';}function rentalFamily(title,count,html,open){return '<details class="rental-family"'+(open?' open':'')+'><summary><span>'+title+'</span><strong>'+count+'</strong></summary><div class="rental-family-body">'+html+'</div></details>';}function buildAllRentalsDrawerHtml(){var objects=store.getState().objects,html='<div class="all-rentals-intro"><strong>Complete rental library</strong><p>Browse everything currently supported from the live catalog. Games, concessions, effects, cooling, power and event accessories are placeable just like tables and inflatables.</p></div><label class="rental-search-box"><span>Search all rentals</span><input type="search" autocomplete="off" placeholder="Bounce house, foam machine, cooler, speaker…" data-role="rental-search"></label><div class="rental-search-empty" data-role="rental-search-empty" hidden>No matching rentals. Try a different name or category.</div>';html+=rentalFamily('Inflatables',INFLATABLES.length,INFLATABLES.length?inflatableCards(INFLATABLES,objects):'<p class="equipment-note">No inflatables are mapped in this catalog.</p>',true);html+=rentalFamily('Games, Concessions, Effects & More',ACCESSORIES.length,accessoryDrawer(ACCESSORIES,objects),true);html+=rentalFamily('Tables',TABLES.length,buildTablesDrawerHtml(),false);html+=rentalFamily('Chairs',CHAIRS.length,buildChairsDrawerHtml(),false);html+=rentalFamily('Tents',TENTS.length,buildTentDrawerHtml(),false);html+=rentalFamily('Dance Floor',DANCE_FLOOR_SIZES.length+2,buildDanceDrawerHtml(),false);html+=rentalFamily('Lighting',LIGHTING_OPTIONS.length,buildLightingDrawerHtml(),false);return html;}function filterRentalLibrary(value){var root=$('drawerBody');if(!root)return;var q=String(value||'').trim().toLowerCase(),families=[].slice.call(root.querySelectorAll('.rental-family')),totalVisible=0;families.forEach(function(family){var cards=[].slice.call(family.querySelectorAll('.item-card,.equipment-card,.accessory-card')),matched=0;if(!q){cards.forEach(function(card){card.hidden=false;});family.hidden=false;return;}cards.forEach(function(card){var hay=(card.textContent||'').toLowerCase();var hit=hay.includes(q);card.hidden=!hit;if(hit){matched++;totalVisible++;}});family.hidden=matched===0;if(matched){family.open=true;family.querySelectorAll('.rental-category').forEach(function(section){var any=[].slice.call(section.querySelectorAll('.item-card,.equipment-card')).some(function(card){return !card.hidden;});section.hidden=!any;});}});if(!q)totalVisible=root.querySelectorAll('.item-card,.equipment-card').length;var empty=root.querySelector('[data-role="rental-search-empty"]');if(empty)empty.hidden=totalVisible>0;}function buildTentDrawerHtml(){
...
function eventSummary(){return summarizeEvent(getScene(),{tabletop:TABLETOP,inflatables:INFLATABLES,accessories:ACCESSORIES,tents:TENTS,tables:TABLES,chairs:CHAIRS,linens:LINENS,lighting:LIGHTING_OPTIONS,danceSection:DANCE_SECTION,tentLightingPrice:tentLightingPriceFor,lightingForTent:lightingForTent});}
...
function renderInspector(conflicts){
  ...
  }else if(item.kind==='accessory'){var accessory=accessoryById(item.accessoryId);if(accessory)renderEquipmentContent(p,accessoryInspector(item,accessory));
  }else{
...
function startPlacement(objects,label,previewId,extra){if(!requireEventEditing())return false;
  ...
  $('placementName').textContent=label;$('placementRotate').hidden=!['table','inflatable','chair','accessory'].includes(objects[0].kind);
...
$('placementRotate')?.addEventListener('click',function(){
  ...
  else{if(['inflatable','chair','accessory'].includes(item.kind))item.rotationDeg=((item.rotationDeg||0)+90)%360;var w=item.widthFt;item.widthFt=item.depthFt;item.depthFt=w;p.widthFt=item.widthFt;p.depthFt=item.depthFt;}
  movePlacement(cx,cy);
});
...
function buildPartyScene(){if(!requireEventEditing())return false;
  if(store.getState().objects.some(o=>!['inflatable','accessory'].includes(o.kind))){showLayoutNotice('Your layout is already started. Use Suggest to replace it, or keep adding items.');return false;}
