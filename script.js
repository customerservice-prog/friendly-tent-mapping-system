    ...
    import('./js/ui/view3d.js?v=20260925-space-scan-5').then(function(mod){
      var snap=view3dPendingSnapshot||buildSnapshot(getConflicts()),inst=mod.init(canvas,{onSelect:handleSelect,onMove:handleMove,onPhotoMove:handlePhotoPlacement,onPlacementMove:movePlacement,onPlace:confirmPlacement,onWalkMode:function(value){setPhoto3dModeUi(value?'walk':'360');},onMeasureMode:function(value){setMeasureUi(value);},onMeasurement:function(value){setMeasurementResult(value);},onScanReconstruction:function(info){
        ...
        if(info?.ready){
          ...
          var m=info.metrics||{},views=m.viewCount||info.sourceFrames?.length||3,agreement=m.multiReferenceAgreementPct??m.multiViewAgreementPct,tier=(m.qualityTier||'').replace(/^./,function(x){return x.toUpperCase();});
          showLayoutNotice('3D Scan'+(tier?' · '+tier+' detail':'')+' · '+views+' real views · '+(m.coveragePct||0)+'% depth coverage'+(agreement!=null?' · '+agreement+'% cross-view agreement':'')+' · '+(m.triangles||0)+' connected surface triangles.',7000);
...
function setPhoto3dModeUi(mode){
  ...
  if($('canvasHint')&&state.viewMode==='3d'&&state.backgroundPhoto){
    ...
    var views=runtime?.metrics?.viewCount||runtime?.sourceFrames?.length||3,agreement=runtime?.metrics?.multiReferenceAgreementPct??runtime?.metrics?.multiViewAgreementPct,tier=runtime?.metrics?.qualityTier;
    $('canvasHint').textContent=isWalk?'Walk Scan · measured Space Scan · WASD / arrow keys to move · drag to look · Esc exits':isOrbit&&scanReady?('3D Scan'+(tier?' · '+tier+' detail':'')+' · '+views+' real views'+(agreement!=null?' · '+agreement+'% spatial agreement':'')+' · orbit limited to captured geometry'):runtime?.loading?'Building high-detail metric depth from the scan…':scanReady?'Matched View · choose 3D Scan for measured depth':'Photo Match · one image is not treated as 360 · record a Space Scan for real 3D depth';
  }
}
