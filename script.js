...
function pickDefaultRoundTable(){var exact=byId(TABLES,'round-5ft');if(exact)return exact;var round=TABLES.filter(function(t){return t.shape==='round'&&t.seatsDefault>0;});if(round.length)return round[0];var any=TABLES.filter(function(t){return t.seatsDefault>0;});return any.length?any[0]:(TABLES.length?TABLES[0]:null);}function useRecommendedLayout(){if(!requireEventEditing())return false;
  ...
  positions.slice(0,needed).forEach(function(pos){objects.push({id:newItemId(),kind:'table',tableId:tableDef.id,shape:tableDef.shape,widthFt:tableDef.diameterFt||tableDef.widthFt,depthFt:tableDef.diameterFt||tableDef.depthFt,...(tableDef.shape==='half-round'?{modelWidthFt:tableDef.widthFt,modelDepthFt:tableDef.depthFt,footprintOriented:true,rotationDeg:0,seatingLayout:tableDef.seatingLayout||'sweetheart'}:{}),x:pos.x,y:pos.y,seatCount:seats,chairId:state.chairId,linenId:null});});
...
$('placementRotate')?.addEventListener('click',function(){
  ...
  else{if(item.shape==='half-round'||['inflatable','chair','equipment','accessory'].includes(item.kind))item.rotationDeg=((item.rotationDeg||0)+90)%360;var w=item.widthFt;item.widthFt=item.depthFt;item.depthFt=w;p.widthFt=item.widthFt;p.depthFt=item.depthFt;}
  movePlacement(cx,cy);
});
