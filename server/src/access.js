...
async function resolveAccess({design,tenant,isStaff}){
  ...
  // Permanent product rule while Friendly Party Rental is the launch tenant:
  // every Friendly customer gets the complete designer free. This is kept in
  // server authority as well as DB policy so a missed migration cannot expose
  // Friendly customers to a RentSketch paywall.
  if(tenant.slug==='friendly' || (tenant.customer_access||'paid')==='free')return {context:'tenant_customer',access:'included',reason:'tenant_free',expiresAt:null,capabilities:FULL_CAPABILITIES,paymentRequired:false,price:null,currency:'usd',tenant:tenant.slug};
  ...
  return {context:'tenant_customer',access:'preview',reason:'tenant_paid_pass',expiresAt:null,capabilities:['view'],paymentRequired:true,price:tenantPrice,currency:'usd',tenant:tenant.slug};
}
