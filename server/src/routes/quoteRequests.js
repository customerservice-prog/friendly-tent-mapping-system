...
function finite(v,min,max){const n=Number(v);return Number.isFinite(n)?Math.max(min,Math.min(max,n)):null;}
...
router.post('/:slug/quote-requests',async(req,res)=>{
 ...
 const body=req.body||{},designId=text(body.designId,80)||null,customerName=text(body.customerName,160),customerEmail=text(body.customerEmail,254),customerPhone=text(body.customerPhone,50),eventDate=text(body.eventDate,20),eventType=text(body.eventType,100),notes=text(body.notes,8000),guestCount=Number.isFinite(Number(body.guestCount))?Math.max(1,Math.min(100000,Number(body.guestCount))):null,estimateTotal=Number.isFinite(Number(body.estimateTotal))?Math.max(0,Math.min(100000000,Number(body.estimateTotal))):null,lineItems=sanitizeLineItems(body.lineItems);
