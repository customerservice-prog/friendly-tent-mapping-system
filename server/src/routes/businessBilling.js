...
async function requireBillingAccess(req,res,next){
  try{
    ...
    if(!payload.isPlatformAdmin){
      ...
      if(!['owner','admin'].includes(membership.rows[0].role)) return res.status(403).json({error:'Only a tenant owner or admin can manage billing'});
    }
    req.tenant=tenant; req.user=payload; next();
...
router.get('/:slug/billing/status', requireBillingAccess, async(req,res)=>{
  const t=req.tenant;
  const sub=await db.query(`SELECT plan_id,status,billing_interval,current_period_start,current_period_end,cancel_at_period_end
    FROM subscriptions WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT 1`,[t.id]);
  res.json({
...
router.post('/:slug/billing/checkout-session', requireBillingAccess, async (req,res)=>{
  ...
  if(tenant.slug==='friendly') return res.status(400).json({error:'Friendly Party Rental currently has free RentSketch access.'});
...
router.post('/:slug/billing/portal-session', requireBillingAccess, async(req,res)=>{
  ...
  const tenant=req.tenant;
  if(tenant.slug==='friendly') return res.status(400).json({error:'Friendly Party Rental currently has free RentSketch access.'});
  if(!tenant.stripe_billing_customer_id) return res.status(409).json({error:'No billing account exists yet. Choose a plan first.'});
  const session=await stripe.billingPortal.sessions.create({customer:tenant.stripe_billing_customer_id,return_url:`${safeOrigin(req)}/dashboard/?billing=portal-return`});
  res.json({url:session.url});
});
