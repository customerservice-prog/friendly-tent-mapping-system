...
router.post('/:slug/designs/:id/background-photo', rawPhoto, wrap(async (req, res) => {
  ...
  const result = await db.query(
    ...
    [design.id, tenant.id, permission.ownerSession, tokenHash(accessToken), detected, req.body.length, req.body]
  );
  // Keep a few recent replacements so a transient autosave failure cannot make
  // the previous background disappear. Old replacements are pruned automatically.
  await db.query(
  ...
  const path = '/api/tenants/' + encodeURIComponent(tenant.slug) + '/background-photo/' + encodeURIComponent(id) + '?t=' + encodeURIComponent(accessToken);
  res.status(201).json({ id, path, mimeType: detected, byteSize: req.body.length, createdAt: result.rows[0].created_at });
}));

router.get('/:slug/background-photo/:photoId', wrap(async (req, res) => {
  const tenant = await tenantForSlug(req.params.slug);
  if (!tenant) return res.status(404).end();
  const supplied = clean(req.query.t, 200);
  if (!supplied) return res.status(404).end();
  const row = (await db.query(
